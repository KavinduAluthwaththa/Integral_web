...
interface ProductEditorPanelProps {
  ...
  onSizeChartUpload: (files: FileList | null) => void;
  onRemoveSizeChartImage: (index: number) => void;
  onMoveSizeChartImage: (index: number, direction: 'up' | 'down') => void;
  onReplaceSizeChartImage: (index: number, file: File | null) => void;
  onSave: () => void;
  onReset: () => void;
}
...
export function ProductEditorPanel({
  ...
  onMoveSizeChartImage,
  onReplaceSizeChartImage,
  onSave,
  onReset,
}: ProductEditorPanelProps) {
  const productImages = parseImages(form.images);
  const sizeChartImages = parseSizeCharts(form.size_chart_images);
    ...
    <section className="border-2 border-foreground/40">
      ...
      <div className="space-y-4 px-lg py-lg">
        <div className="space-y-2">
          <label className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Product ID</label>
          <Input
            value={form.sku}
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div className="flex items-center gap-3">
            <Checkbox
              ...
              onCheckedChange={(value) => onFormChange({ is_limited_edition: Boolean(value) })}
              className="h-4 w-4 rounded-none border-2 border-foreground/40 bg-background focus-visible:ring-0 focus-visible:ring-offset-0 data-[state=checked]:bg-foreground data-[state=checked]:text-background"
            />
            <label htmlFor="is-limited" className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <div className="flex items-center gap-3">
            <Checkbox
              ...
              onCheckedChange={(value) => onFormChange({ is_hidden: Boolean(value) })}
              className="h-4 w-4 rounded-none border-2 border-foreground/40 bg-background focus-visible:ring-0 focus-visible:ring-offset-0 data-[state=checked]:bg-foreground data-[state=checked]:text-background"
            />
            <label htmlFor="is-hidden" className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Product Images (display order)</label>
            <span className="text-[11px] text-muted-foreground">First image shows on cards</span>
          </div>
          <input
            type="file"
          ...
          <p className="text-xs text-muted-foreground">
            {uploadingImages
              ? 'Uploading images...'
              : 'Preview order matches the storefront gallery. Use Move to reorder.'}
          </p>
          <div className="space-y-2">
            {productImages.length === 0 ? (
              ...
              productImages.map((image, index) => (
                <div
                  key={`${image}-${index}`}
                  className="flex flex-col items-start gap-3 border-2 border-foreground/40 px-3 py-2 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div className="flex items-center gap-3 w-full sm:w-auto sm:flex-1 min-w-0">
                    <span className="text-[11px] font-mono text-muted-foreground">#{index + 1}</span>
                    <div className="relative h-16 w-16 overflow-hidden border border-foreground/20 bg-secondary">
                      <Image
                        src={image}
                    ...
                    <span className="truncate text-xs text-muted-foreground max-w-full sm:max-w-[420px] break-all">{image}</span>
                  </div>
                  <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:flex-nowrap">
                    <button
                      onClick={() => onMoveImage(index, 'up')}
                      disabled={index === 0}
                      className="inline-flex h-8 items-center justify-center border-2 border-foreground/40 px-3 text-[10px] uppercase tracking-[0.2em] text-foreground transition-colors duration-300 hover:bg-foreground hover:text-background disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      Move Up
                    </button>
                    <button
                    ...
                    <label className="inline-flex h-8 cursor-pointer items-center justify-center border-2 border-foreground/40 px-3 text-[10px] uppercase tracking-[0.2em] text-foreground transition-colors duration-300 hover:bg-foreground hover:text-background">
                      Replace
                      <input
                        type="file"
                        accept="image/png,image/jpeg,image/webp,image/avif"
                    ...
                    <button
                      ...
                      className="inline-flex h-8 items-center justify-center border-2 border-red-600/40 px-3 text-[10px] uppercase tracking-[0.2em] text-red-700 transition-colors duration-300 hover:bg-red-600 hover:text-background"
                    >
                      Remove
                    </button>
                  </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Size Chart Images</label>
            <span className="text-[11px] text-muted-foreground">Shown on size chart page</span>
          </div>
          <input
            type="file"
            accept="image/png,image/jpeg,image/webp,image/avif"
            multiple
            onChange={(event) => onSizeChartUpload(event.target.files)}
            disabled={uploadingSizeCharts}
            className="block w-full border-2 border-foreground/40 bg-background px-md py-sm text-sm text-foreground transition-colors focus-visible:bg-input focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 file:mr-md file:border-2 file:border-foreground file:bg-background file:px-md file:py-xs file:text-xs file:uppercase file:tracking-[0.2em] file:text-foreground file:transition-colors hover:file:bg-foreground hover:file:text-background disabled:cursor-not-allowed disabled:opacity-50"
          />
          <p className="text-xs text-muted-foreground">
            {uploadingSizeCharts
              ? 'Uploading size charts...'
              : 'Upload one or more size chart images. Order is preserved on the size chart page.'}
          </p>
          <div className="space-y-2">
            {sizeChartImages.length === 0 ? (
              <p className="text-xs text-muted-foreground">No size charts uploaded yet.</p>
            ) : (
              sizeChartImages.map((image, index) => (
                ...
                >
                  ...
                  <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:flex-nowrap">
                    <button
                      onClick={() => onMoveSizeChartImage(index, 'up')}
                      disabled={index === 0}
                      className="inline-flex h-8 items-center justify-center border-2 border-foreground/40 px-3 text-[10px] uppercase tracking-[0.2em] text-foreground transition-colors duration-300 hover:bg-foreground hover:text-background disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      Move Up
                    ...
                    <button
                      onClick={() => onMoveSizeChartImage(index, 'down')}
                      disabled={index === sizeChartImages.length - 1}
                      className="inline-flex h-8 items-center justify-center border-2 border-foreground/40 px-3 text-[10px] uppercase tracking-[0.2em] text-foreground transition-colors duration-300 hover:bg-foreground hover:text-background disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      Move Down
                    </button>
                    <label className="inline-flex h-8 cursor-pointer items-center justify-center border-2 border-foreground/40 px-3 text-[10px] uppercase tracking-[0.2em] text-foreground transition-colors duration-300 hover:bg-foreground hover:text-background">
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          const file = event.target.files?.[0] || null;
                          onReplaceSizeChartImage(index, file);
                          event.currentTarget.value = '';
                        }}
                        disabled={uploadingSizeCharts}
                      />
